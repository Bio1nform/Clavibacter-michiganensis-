...
# debugged on 16th november       , 2012 by patrick kueck -> partition range bug. if alignmnet file contains only one base position, wrong partition range counting of the infofile. Changed in line 592
...
<<preface
	...
	Author      : Patrick K?ck, ZFMK Bonn, GERMANY
	...
	sub print_out{
		...
		sub nexus_block{
			...
			for my $nex_tax ( sort {$a<=>$b} keys %$href_seq_nex ){
				...
				if ( @$aref_sub_parts_nex  ){ my $string_nex = join ( "", @$aref_sub_parts_nex ) ; push @$aref_block_rows_nex , $string_nex }
				if ( @$aref_block_rows_nex ){ push @{ $seq_parts_nex{$nex_tax}} , ( my $string_row_nex = join ( " ", @$aref_block_rows_nex ) ) ; $href_counter_nex->{blocks}++ }
			}
